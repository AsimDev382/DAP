        ...
        <div class="col-12 mb-4 case-bar ">
            <div class="row d-flex align-items-center justify-content-between  gap-3">
                ...
                <div class="col-md-1 text-nowrap">
                    <p class="fw-bold fs-5 me-0 m-0">Client Reports</p>
                </div>

                <!-- Search -->
                <div class="col-md-6">
                    <div class="client-search flex-grow-1">
                ...
                <div class="col-md-3 d-flex justify-content-end">
                    <div class="d-flex gap-3 align-items-center ">
                        ...
                        @can('create client report')
                            <a href="{{ route('client-report.create') }}" type="submit" class="btn btn-primary px-4 py-2">Report</a>
                        @endcan
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody id="ReportTableBody">
                        ...
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>{{ $report->user->auto_id }}</td>
                            <td>{{ $report->client_id }}</td>
                            <td><a href="{{ route('client-report.show', $report->id) }}">{{ $report->client_name }}</a></td>
                            <td>{{ @$report->company->company_name }}</td>
                            <td>{{ @$report->brand->brand_name }}</td>
                            <td>{{ @$report->product->product_name }}</td>
                            <td>{{ $report->payed_amount }}</td>
                            ...
                            <td>
                                @can('edit client report')
                                <a href="#"
                                    data-bs-toggle="modal"
                                        data-bs-target="#confirmEditModal"
                                        data-url="{{ route('client-report.edit', $report->id) }}"><i class="bi bi-pencil-square"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                @endcan
                                @can('view client report')
                                <a href="{{ route('client-report.show', $report->id) }}"><i class="bi bi-eye"></i></a>
                                @endcan
                        ...
                        <div class="modal fade" id="deleteModal{{ $report->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <form action="{{ route('client-report.destroy', $report->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                    ...
                    $.each(response, function(index, report) {
                        ...
                        $('#ReportTableBody').append(`
                            <tr>
                                ...
                                <td>${statusBadge}</td>
                                <td>
                                    <a href="/client-report/edit/${report.id}"><i class="bi bi-pencil-square"></i></a>
                                    <button data-bs-toggle="modal" data-bs-target="#deleteModal${report.id}">
                                        <i class="bi bi-trash text-danger ms-2"></i>
                                    </button>
                                </td>
