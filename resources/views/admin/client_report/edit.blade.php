...
<div class="main-content">
    ...
    <!-- Form -->
    <form id="mainForm" action="{{ route('client-report.update', $report->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
        @csrf
        @method('PUT')
        <div class="row px-4">
            <div class="col-12 mb-4 ">
                <!-- Title -->
                <div class="fw-bold fs-4 me-3">Edit Client Report</div>
            </div>
                ...
                <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="">
                                    ...
                                    <select name="user_id" id="UserId" class="form-select">
                                        ...
                                        <option value="{{ $user->id }}" {{ $user->id == $report->user_id ? 'selected': '' }}>{{ $user->auto_id }}</option>
                                        @endforeach
                                    </select>
                                    {{-- <input type="text" name="auto_id" class="form-control" value="{{ $latestUser }}" readonly id="name"> --}}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row d-flex align-items-baseline">
                            <div class="col-md-12">
                                <div class="">
                                    ...
                                    <input type="text" id="UserClientId" value="{{ $report->client_id }}" class="form-control" readonly>
                                    <input type="hidden" name="client_id" value="{{ $report->client_id }}" id="hideUserClientId">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="row d-flex align-items-baseline">
                            <div class="col-md-12">
                                <div class="">
                                    <label class="eighteenblack mb-2">Client Name*</label>
                                    <input type="text" id="UserClientName" value="{{ $report->client_name }}" class="form-control" readonly>
                                    <input type="hidden" name="client_name" value="{{ $report->client_name }}" id="hideUserClientName">
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <input type="text" name="total_amount" value="{{ $report->total_amount }}" class="form-control" placeholder="Enter Total Amount">
                    @error('total_amount')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="eighteenblack mb-2">Payed Amount*</label>
                    <input type="text" name="payed_amount" value="{{ $report->payed_amount }}" class="form-control" placeholder="Enter Payed Amount">
                    @error('payed_amount')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    <label class="eighteenblack mb-2">Balance*</label>
                    <input type="text" name="balance" value="{{ $report->balance }}" class="form-control" placeholder="Enter Balance">
                    @error('balance')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-12">
             <div class="row d-flex justify-content-between">
                ...
                <div class="col-md-6 text-end"> <a href="{{ route('client-report.index') }}" class="btn btn-cancel me-2">Cancel</a>
...
<script>
    $(document).ready(function() {
        $('#UserId').change(function() {
            ...
            if (UserId) {
                $.ajax({
                    url: '/client-report/get-user-data/' + UserId,
                    type: 'GET'
                    , success: function(response) {
                        console.log(response);

                        $('#UserClientId').val(response.auto_id);
                        $('#UserClientName').val(response.name);

                        $('#hideUserClientId').val(response.auto_id);
                        $('#hideUserClientName').val(response.name);
                    }
                    , error: function() {
