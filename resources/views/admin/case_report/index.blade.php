        ...
        <div class="col-12 mb-4 case-bar ">
            <div class="row d-flex align-items-center justify-content-between  gap-3">
                ...
                <div class="col-md-1 text-nowrap">
                    <p class="fw-bold fs-5 me-0 m-0">Case Reports</p>
                </div>
                ...
                <div class="col-md-3 d-flex justify-content-end">
                    <div class="d-flex gap-3 align-items-center ">
                        ...
                        @can('create case report')
                            <a href="{{ route('case-report.create') }}" type="submit" class="btn btn-primary px-4 py-2">New invoice</a>
                        @endcan
                    </div>
                </div>
            </div>
                ...
                <table class="table table-hover mb-0">
                    ...
                    <tbody id="ReportTableBody">
                        ...
                        <tr>
                            ...
                            {{-- <td>
                                <a href="{{ route('case-report.show', $report->id) }}">{{ $report->cases->case_name }}</a>
                            </td> --}}
                            ...
                            <td>
                                @can('edit case report')
                                <a href="#"
                                    data-bs-toggle="modal"
                                        data-bs-target="#confirmEditModal"
                                        data-url="{{ route('case-report.edit', $report->id) }}"><i class="bi bi-pencil-square"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                @endcan
                                @can('view case report')
                                <a href="{{ route('case-report.show', $report->id) }}"><i class="bi bi-eye"></i></a>
                                @endcan
                        ...
                        <div class="modal fade" id="deleteModal{{ $report->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <form action="{{ route('case-report.destroy', $report->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                    ...
                    $.each(response, function(index, report) {
                        ...
                        $('#ReportTableBody').append(`
                            <tr>
                                ...
                                <td>${statusBadge}</td>
                                <td>
                                    <a href="/case-report/edit/${report.id}"><i class="bi bi-pencil-square"></i></a>
                                    <button data-bs-toggle="modal" data-bs-target="#deleteModal${report.id}">
                                        <i class="bi bi-trash text-danger ms-2"></i>
                                    </button>
                                </td>
