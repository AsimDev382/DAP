...
<div class="main-content">
    ...
    <form id="mainForm" action="{{ route('department.update', $department->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="row px-4">
            <div class="col-12 mb-4 ">
                <!-- Title -->
                <div class="fw-bold fs-4 me-3">Edit Department </div>
            </div>
            <div class="col-md-12">
                <div>
                    ...
                    <select name="company_id" class="form-control" id="">
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}" {{ $company->id == $department->company_id ? 'selected' : '' }}>{{ $company->company_name }}</option>
                        @endforeach
                    </select>
                    @error('company_id')
            ...
            <div class="col-md-4">
                <div class="my-3">
                    ...
                    <input type="text" name="name" value="{{ $department->name }}" class="form-control" id="name" placeholder="Enter Department Name">
                    @error('name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-4">
                <div class="my-3">
                    ...
                    <input type="text" name="head_name" value="{{ $department->head_name }}" class="form-control" id="name" placeholder="Enter Department Head Name">
                    @error('head_name')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-4">
                <div class="my-3">
                    ...
                    <input type="text" name="location" value="{{ $department->location }}" class="form-control" id="name" placeholder="Enter Department Location">
                    @error('location')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
