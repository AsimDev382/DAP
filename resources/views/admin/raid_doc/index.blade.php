        ...
        <div class="row px-4">
            <div class="col-12 mb-4 case-bar ">
                <div class="row d-flex align-items-center justify-content-between  gap-3">
                    ...
                    <!-- Title -->
                    <div class="col-md-2">
                        <p class="fw-bold fs-5 me-3 m-0 text-nowrap">Raid Documentation</p>
                    </div>
                    ...
                    <div class="col-md-3 d-flex justify-content-end">
                        <div class="d-flex gap-3 align-items-center ">
                            ...
                            @can('create raid plaining & execution')
                                <a href="{{ route('raid.doc.create') }}" type="submit" class="btn btn-primary px-2 py-2">Documents</a>
                            @endcan
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody id="raidTableBody">
                            ...
                            <tr class="table-card-row">
                                ...
                                <td>
                                    @if($raid->document)
                                    <img src="{{ asset('storage/'.$raid->document) }}" width="50px" alt="">
                                    @else
                                        <span>No Iamge</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @can('edit raid documentation')
                                    <a href="#"
                                        data-bs-toggle="modal"
                                        data-bs-target="#confirmEditModal"
                                        data-url="{{ route('raid.doc.edit', $raid->id) }}"><i class="bi bi-pencil-square"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;
                                    @endcan
                                    @can('view raid documentation')
                                    <a href="{{ route('raid.doc.view', $raid->id) }}"><i class="bi bi-eye"></i></a>
                                    @endcan
                                    @can('delete raid documentation')
                            ...
                            <div class="modal fade" id="deleteModal{{ $raid->id }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <form action="{{ route('raid.doc.destroy', $raid->id) }}">
                                    @csrf
                                    @method('DELETE')
...
$(document).ready(function() {
    ...
    function filterGroup(sortOrder) {
        $.ajax({
            url: '{{ url("raid-documentation/sort") }}',
            type: 'GET',
            data: {
            ...
            success: function(response) {
                ...
                if (response.length > 0) {
                    $.each(response, function(index, raids) {
                        ...
                        let logoUrl = raids.document ? `/storage/${raids.document}` : '';
                        ...
                        $('#raidTableBody').append(`
                            <tr>
                                ...
                                <td>${raids.date}</td>
                                <td><img src="${logoUrl}" width="50px;" alt=""></td>
                                <td>
                                    <a href="/raid-documentation/${raids.id}/edit"><i class="bi bi-pencil-square"></i></a>&nbsp;&nbsp;&nbsp;
                                    <a href="/raid-documentation/view/${raids.id}"><i class="bi bi-eye"></i></a>
