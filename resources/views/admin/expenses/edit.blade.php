...
<div class="main-content">
    ...
    <!-- Form -->
    <form id="mainForm" action="{{ route('expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
        @csrf
        @method('PUT')
        <div class="row px-4">
            <div class="col-12 mb-4 ">
                <!-- Title -->
                <div class="fw-bold fs-4 me-3">Edit Expenses</div>
            </div>
                ...
                <div class="row d-flex align-items-baseline">
                    <div class="col-md-4">
                        <div class="my-3">
                            ...
                            <input type="text" name="auto_id" class="form-control" value="{{ $expense->auto_id }}" readonly id="name">
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="my-3">
                            <label class="eighteenblack mb-2">Client ID</label>
                            <input type="text" name="client_id" value="{{ $expense->client_id}}" name="client_id" class="form-control" readonly>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="my-3">
                            ...
                            <select name="case_id" class="form-control">
                                ...
                                <option value="{{ $case->id }}" {{ $expense->case_id == $case->id ? 'selected' : '' }}>{{ $case->case_name }}</option>
                                @endforeach
                            </select>
                            @error('case_id')
            ...
            <div class="col-md-2">
                <div>
                    ...
                    <select class="form-select" name="case_type" aria-label="Default select example">
                        <option selected disabled>Select Case</option>
                        <option value="Targeted" {{ $expense->case_type =='Targeted' ? 'selected' : '' }}>Targeted</option>
                        <option value="Pro Active" {{ $expense->case_type == 'Pro Active' ? 'selected' : '' }}>Pro Active</option>
                    </select>
                    @error('case_type')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                    ...
                    <select name="brand_id" id="brand_id" class="form-select">
                        ...
                        @foreach($brands as $brand)
                            <option value="{{ $brand->id }}"
                                {{ $expense->brand_id == $brand->id ? 'selected' : '' }}>
                                {{ $brand->brand_name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select name="product_id" id="product_id" class="form-select">
                        ...
                        @foreach($products as $product)
                            <option value="{{ $product->id }}"
                                {{ $expense->product_id == $product->id ? 'selected' : '' }}>
            ...
            <div class="col-md-2">
                <div class="mb-3">
                    ...
                    <select name="brand_id" class="form-select" aria-label="Default select example">
                        ...
                        @foreach ($brands as $brand)
                        <option value="{{ $brand->id }}" {{ $expense->brand_id == $brand->id ? 'selected' : '' }}>{{ $brand->brand_name }}</option>
                        @endforeach
                    </select>
                    @error('brand_id')
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="col-md-3">
                <div class="mb-3">
                    ...
                    <select name="product_id" class="form-select" aria-label="Default select example">
                        ...
                        @foreach ($products as $product)
                        <option value="{{ $product->id }}" {{ $expense->product_id == $product->id ? 'selected' : '' }}>{{ $product->product_name }}</option>
                        @endforeach
                    </select>
                    @error('product_id')
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="col-md-3">
                <div class="mt-3">
                    ...
                    <select name="currency_id" class="form-select" aria-label="Default select example">
                        ...
                        <option value="{{ $currency->id }}" {{ $expense->currency_id == $currency->id ? 'selected' : '' }}>{{ $currency->currency_name }}</option>
                        @endforeach
                    </select>
                    @error('currency_id')
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="col-md-2">
                <div class="my-3">
                    ...
                    <select name="case_priority" class="form-select" aria-label="Default select example">
                        ...
                        <option value="High" {{ $expense->case_priority == 'High' ? 'selected' : '' }}>High Priority</option>
                        <option value="Medium" {{ $expense->case_priority == 'Medium' ? 'selected' : '' }}>Medium Priority</option>
                        <option value="Low" {{ $expense->case_priority == 'Low' ? 'selected' : '' }}>Low Priority</option>
            ...
            <div class="col-md-2">
                <div class="my-3">
                    ...
                    <input type="text" name="advance_fee" class="form-control" value="{{ $expense->advance_fee }}" placeholder="Enter Advance Fee">
                    @error('advance_fee')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <input type="text" name="case_expense" class="form-control" value="{{ $expense->case_expense }}" placeholder="Enter Case Expenses">
                    @error('case_expense')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <input type="text" name="total_amount" class="form-control" value="{{ $expense->total_amount }}" placeholder="Enter Total Amount">
                    @error('total_amount')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <input type="text" name="case_location" value="{{ $expense->case_location }}" class="form-control" id="flag" placeholder="Enter Case Location">
                    @error('case_location')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <input type="text" name="start_date" value="{{ $expense->start_date }}" class="form-control datepickerdesign" placeholder="Start Date">
                    @error('start_date')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <input type="text" name="end_date" value="{{ $expense->end_date }}" class="form-control datepickerdesign" placeholder="End Date">
                    @error('end_date')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-3">
                <div class="my-3">
                    ...
                    <select name="status" class="form-select" aria-label="Default select example">
                        ...
                        <option value="In Progress" {{ $expense->status == 'In Progress' ? 'selected' : '' }}>In Progress</option>
                        ...
                        <option value="Closed(Completed)" {{ $expense->status == 'Closed(Completed)' ? 'selected' : '' }}>Closed(Completed)</option>
                        <option value="High-Risk-case" {{ $expense->status == 'High-Risk-case' ? 'selected' : '' }}>High-Risk-case</option>
                        <option value="Reopened Cases" {{ $expense->status == 'Reopened Cases' ? 'selected' : '' }}>Reopened Cases</option>
                    </select>
                    @error('status')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
            ...
            <div class="col-md-12">
             <div class="row d-flex justify-content-between">
                ...
                <div class="col-md-6 text-end"> <a href="{{ route('expenses.index') }}" class="btn btn-cancel me-2">Cancel</a>
...
<script>
    ...
    // On page load (Edit)
    @if(isset($expense))
        loadBrands("{{ $expense->company_id }}", "{{ $expense->brand_id }}", "{{ $expense->product_id }}");
    @endif
});
</script>
@endsection
