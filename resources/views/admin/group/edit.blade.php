...
<div class="main-content">
    ...
    <form id="mainForm" action="{{ route('group.update', $group->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="row px-4">
            <div class="col-12 mb-4">
                <!-- Title -->
                <div class="fw-bold fs-4 me-3">Edit Group </div>
            </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="department_id" class="form-control" id="">
                        ...
                        @foreach ($departments as $department)
                            <option value="{{ $department->id }}" {{ $department->id == $group->department_id ? 'selected' : '' }}>{{ $department->name }}</option>
                        @endforeach
                    </select>
                    @error('department_id')
                        <div class="text-danger">{{ $message }}</div>
                ...
                <div class="mb-3">
                    ...
                    <select name="sub_department_id" class="form-control" id="">
                        ...
                        @foreach ($sub_departments as $sub)
                            <option value="{{ $sub->id }}" {{ $sub->id == $group->sub_department_id ? 'selected' : '' }}>{{ $sub->sub_name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="company_id" class="form-control" id="">
                        ...
                        @foreach ($companies as $company)
                            <option value="{{ $company->id }}" {{ $company->id == $group->company_id ? 'selected' : '' }}>{{ $company->company_name }}</option>
                        @endforeach
                    </select>
                    @error('company_id')
                        <div class="text-danger">{{ $message }}</div>
            ...
            <div class="col-md-4">
                <div class="mb-3">
                    ...
                    <select class="form-select" name="user_id" aria-label="Default select example">
                        ...
                        <option value="{{ $user->id }}" {{ $user->id == $group->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endforeach
                    </select>
                    @error('user_id')
                        <div class="text-danger">{{ $message }}</div>
