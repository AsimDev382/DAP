...
<div class="main-content">
    ...
    <!-- Form -->
    <form id="mainForm" action="{{ route('assign.tasks.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
        @csrf

        <div class="row px-4">
            <div class="col-12 mb-4 ">
                <!-- Title -->
                <div class="fw-bold fs-4 me-3">Add Assign Task</div>
            </div>
            <div class="col-md-10">
                <div class="row d-flex align-items-baseline">
                    @php
                        $latestUser = \App\Models\AssignTask::orderBy('id', 'desc')->first();
                        ...
                        <div class="my-3">
                            ...
                            <select name="case_management_id" class="form-select" aria-label="Default select example">
                                ...
                                @foreach ($cases as $case)
                                    <option value="{{ $case->id }}" {{ old('case_management_id') == $case->id ? 'selected' : '' }}>{{ $case->case_name }}</option>
                                @endforeach
                            </select>
                                        ...
                                        <li>
                                            <div class="form-check dropdown-item">
                                                <input class="form-check-input custom-checkbox" type="checkbox"
                                                    value="{{ $task->id }}" id="5kmdistance" name="task_id[]"
                                                    {{ is_array(old('task_id')) && in_array($task->id, old('task_id')) ? 'checked' : '' }}>
                        ...
                        <ul class="dropdown-menu p-2 px-4" aria-labelledby="dropdownMenuButton">
                            @foreach ($groups as $group)
                                <li>
                                    <div class="form-check dropdown-item">
                                        <input class="form-check-input custom-checkbox" type="checkbox"
                                            value="{{ $group->id }}" id="5kmdistance" name="group_id[]"
                                            {{ is_array(old('group_id')) && in_array($group->id, old('group_id')) ? 'checked' : '' }}>
                                        <label class="form-check-label"
                        ...
                        <ul class="dropdown-menu p-2 px-4" aria-labelledby="dropdownMenuButton">
                            @foreach ($users as $user)
                                <li>
                                    <div class="form-check dropdown-item">
                                        <input class="form-check-input custom-checkbox" type="checkbox"
                                            value="{{ $user->id }}" id="5kmdistance" name="user_id[]"
            ...
            <div class="col-md-4 mt-3">
                <div class="">
                    ...
                    <input type="text" name="assign_date" value="{{ old('assign_date') }}" class="form-control datepickerdesign"
